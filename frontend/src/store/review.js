...
export const createReview = createAsyncThunk(
  ...
  async (reviewData, { rejectWithValue, dispatch }) => {
    try {
      // Validate reviewData
      if (
        typeof reviewData.userId !== "number" ||
        typeof reviewData.itemId !== "number"
      ) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(editReview.fulfilled, (state, action) => {
        ...
        const index = state.reviews[itemId].findIndex(
          (review) => review.id === id
        );
        if (index !== -1) {
      ...
      .addCase(deleteReview.fulfilled, (state, action) => {
        ...
        state.reviews[itemId] = state.reviews[itemId].filter(
