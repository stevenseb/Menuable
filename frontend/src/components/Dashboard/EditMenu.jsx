  ...
  const renderItems = (itemsList) => (
    itemsList.map(item => (
      <div key={item.id} className={styles.itemRow}>
        ...
        />
        <button onClick={() => openDeleteModal(item.id)} className={styles.deleteButton}>Delete</button>
      </div>
    ))
  );

  const onMenuItems = items.filter(item => item.onMenu).sort((a, b) => a.name.localeCompare(b.name));
  const offMenuItems = items.filter(item => !item.onMenu).sort((a, b) => a.name.localeCompare(b.name));

  return (
    <div className={styles.container}>
      <div className={styles.onMenuSection}>
      <h2 className={styles.sectionTitle}>On Menu</h2>
        <p className={styles.smallText2}>Total number of items on menu: {onMenuItems.length}</p>
        <p className={styles.smallText}>Check = add to menu &nbsp;&nbsp;    ||  &nbsp;&nbsp;   Uncheck = remove from menu</p>
        {renderItems(onMenuItems)}
      </div>
      <div className={styles.offMenuSection}>
      <h2 className={styles.sectionTitle}>Off Menu</h2>
      <p className={styles.smallText2}>Total number of items off menu: {offMenuItems.length}</p>
