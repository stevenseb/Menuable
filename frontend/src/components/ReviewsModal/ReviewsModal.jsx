...
const ReviewsModal = ({ item }) => {
  ...
  return (
    <div className="reviews-modal">
      <h2>{item.name}</h2>
      <h4>Average rating: <span className="average-rating">
        {averageRating} <FontAwesomeIcon icon={faStar} style={{ color: 'gold' }} />
      </span></h4>
      <div className="reviews-container">
        {sortedReviews.map((review) => (
          <div key={review.id} className="review">
            <div className="review-header">
              <span className="silver">{review.rating} <FontAwesomeIcon icon={faStar} style={{ color: 'gold' }} /></span>
              <span className="silver">{new Date(review.createdAt).toLocaleDateString()}</span>
            </div>
            ...
            {loggedInUser && loggedInUser.id === review.userId && (
              <div className="review-actions">
                {editingReview === review.id ? (
                  <button onClick={() => handleSave(review)} className="save">Save</button>
                ) : (
